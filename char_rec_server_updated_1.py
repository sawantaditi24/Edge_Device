...
OUTPUT_DIR = "/home/aditi/Character_Recognition_Files/outputs"

@app.route('/text-status', methods=['GET'])
def text_status():
    try:
        # Fetch files saved by the backend
        files = [f for f in os.listdir(OUTPUT_DIR) if f.startswith('text_detected_')]
        if not files:
            return jsonify({"message": "No text detected", "imageUrl": "", "text": "", "timestamp": None})
        ...
        if time.time() - os.path.getctime(latest_path) < 5:
            return jsonify({
                ...
                "imageUrl": f"http://192.168.4.229:5000/text-image/{latest_file}",
...
@app.route('/text-image/<filename>', methods=['GET'])
def text_image(filename):
